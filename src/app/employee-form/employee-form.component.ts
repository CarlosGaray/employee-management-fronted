...
@Component({
  ...
  template: `
    <form class="employee-form" autocomplete="off" [formGroup]="employeeForm" (ngSubmit)="submitForm()">
      ...
      <div class="form-floating mb-3">
        <input type="text" class="form-control" formControlName="position" placeholder="Position" required>
      ...
      <div *ngIf="position.invalid && (position.dirty || position.touched)" class="alert alert-danger">
        ...
        Position must be atleast 5 characters
        </div>
      </div>
      <div class="mb-3">
        <div class="form-check">
          <input type="radio" class="form-check-input" formControlName="level" name="level" id="level-junior" value="junior" required>
          <label class="form-check-label" for="level-junior">Junior</label>
        </div>
        <div class="form-check">
          <input type="radio" class="form-check-input" formControlName="level" name="level" id="level-mid" value="mid" required>
          <label class="form-check-label" for="level-mid">Mid</label>
        </div>
        <div class="form-check">
          <input type="radio" class="form-check-input" formControlName="level" name="level" id="level-senior" value="senior">
